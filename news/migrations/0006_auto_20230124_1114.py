# Generated by Django 3.2.4 on 2023-01-24 11:14
...
def check_and_escape(editor_type, instance):
    try:
        ...
        json.loads(str(getattr(instance,editor_type).raw_text))
    except json.decoder.JSONDecodeError as e:
        # If the parse fails, escape any invalid characters
        getattr(instance,editor_type).raw_text = json.dumps(str(getattr(instance,editor_type).raw_text))

def escape_characters(apps, schema_editor):
    news = apps.get_model('news', 'NewsPage')
    for news_instance in news.objects.all():
        if news_instance.content_editor.raw_text:
            check_and_escape('content_editor', news_instance)
        if news_instance.content_editor_en.raw_text:
            check_and_escape('content_editor_en', news_instance)
        if news_instance.content_editor_fr.raw_text:
            check_and_escape('content_editor_fr', news_instance)
        if  news_instance.content_editor_pt.raw_text:
            check_and_escape('content_editor_pt', news_instance)
        news_instance.save()
